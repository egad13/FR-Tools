...
const HatchCalc = (function () {
	...
	function geneProbability(parent1, parent2, goal) {
		...
		for (const slot of Object.keys(goal.gene)) {
			...
			if (goal.gene[slot] !== parent1.gene[slot] && goal.gene[slot] !== parent2.gene[slot]) {
				...
				n *= FRdata.calcRarityProb(FRdata.gene[slot], parent1.gene[slot], parent2.gene[slot], goal.gene[slot]);
			}
		}
		if (errs.length > 0) {
			return errs;
	...
	/** Calculates a detailed report of the probability of hatching the given Goal
	 * hatchling by nesting the given parents. Includes the overall probability of a goal
	 * hatchling occurring in one egg and in one nest, tables of expected probabilities
	 * within several attempts of eggs and nests, and probabilities of success in
	 * individual traits.
	 * @param {{breed:number, colour:{primary: number, secondary: number, tertiary: number}, gene:{primary: number, secondary: number, tertiary: number}}} parent1
	 * @param {{breed:number, colour:{primary: number, secondary: number, tertiary: number}, gene:{primary: number, secondary: number, tertiary: number}}} parent2
	 * @param {{breed:number, colour:{primary: number, secondary: number, tertiary: number}, gene:{primary: number, secondary: number, tertiary: number}, eye:number, gender:number}} goal
	 * @returns {{breed: number,colour:number, gene:number, eye:number, gender:number, overall:number, per_nest:number, avg_nest_size:number, egg_table:number[][], nest_table:number[][]}|{err:string[]}}
	 * @memberof module:HatchCalc
	 */
	 ...
	 * @param {{breed: number,colour:number, gene:number, eye:number, gender:number, overall:number, per_nest:number, avg_nest_size:number, egg_table:number[][], nest_table:number[][]}} result
	 * @returns {string}
	 * @memberof module:HatchCalc */
